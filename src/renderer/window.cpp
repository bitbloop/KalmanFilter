...
namespace
{
	...
	bool _window_kill_requested{ false };
};
...
namespace
{
	...
	bool create_window()
	{
		...
		_window_width = mode->width / 2;
		_window_height = mode->height / 2;
		//_window = glfwCreateWindow(_window_width, _window_height, "Tracking", monitor, NULL);
		_window = glfwCreateWindow(_window_width, _window_height, "Tracking", NULL, NULL);
		if (!_window) {
...
{
	return glfwWindowShouldClose(_window) || _window_kill_requested;
}

/** */
GLFWwindow* renderer::window::get_window()
{
	return _window;
}
